...
const Sector = () => {
  ...
  useEffect(() => {
    const fetchForesightData = async () => {
      try {
        ...
        setForesightData(data.listForesightData.items.sort((a, b) => a.id - b.id));
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "Scenario 1",
        data: regions.map(region => foresightData.find(item => item.region === region && item.scenario === "scen1").value),
      ...
      {
        label: "Scenario 2",
        data: regions.map(region => foresightData.find(item => item.region === region && item.scenario === "scen2").value),
